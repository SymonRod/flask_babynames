...
ringcover = new THREE.Mesh(new THREE.PlaneGeometry(50, 15, 1), new THREE.MeshBasicMaterial({ color: 0x222222, opacity: 0, transparent: true }));
...
(function() {
    var plain, i;
    for (i = 0; i < 10; i++) {
        plain = new THREE.Mesh(new THREE.PlaneGeometry(length * 2 + 1, radius * 3, 1), new THREE.MeshBasicMaterial({ color: 0x222222, transparent: true, opacity: 0.13 }));
...
function render() {
    ...
    animatestep = Math.max(0, Math.min(240, toend ? animatestep + 3 : animatestep - 4));
